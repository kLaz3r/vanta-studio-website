...
export function StepServices({ onNext, onBack, defaultValues }: StepServicesProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onNext)} noValidate>
      <div className="mb-8">
        <h2 className="text-2xl font-bold text-vanta-light">Servicii dorite</h2>
      ...
      <div className="space-y-5">
        <div>
          <label className="mb-1.5 block text-sm font-medium text-vanta-light/80">
            Servicii
          </label>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {SERVICE_OPTIONS.map((service) => (
              <button
                ...
                className={cn(
                  "rounded-xl border px-3 py-2.5 text-left text-sm transition-all duration-200 cursor-pointer",
